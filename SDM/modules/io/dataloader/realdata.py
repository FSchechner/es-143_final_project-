    ...
    def load(self, objdir, prefix, margin = 0, max_image_resolution = 2048, aug=[]):
        ...
        for i, indexofimage in enumerate(indexset):
            ...
            if i == 0:
                ...
                mask_candidates = [
                    mask_path,
                    os.path.join(img_dir, 'masks', 'object_mask.png'),
                    os.path.join(img_dir, 'masks', 'mask.png'),
                ]
                mask_found = None
                ...
                if mask_found is not None and i == 0:
                    mask = (cv2.imread(mask_found, flags = cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH) > 0).astype(np.float32)
                    if len(mask.shape) == 3:
                        mask = mask[:, :, 0]
                    mask_flag = True
